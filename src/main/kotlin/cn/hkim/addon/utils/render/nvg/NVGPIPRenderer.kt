...
// Origin: https://github.com/odtheking/Odin/blob/main/src/main/kotlin/com/odtheking/odin/utils/ui/rendering/NVGPIPRenderer.kt
...
class NVGPIPRenderer(vertexConsumers: MultiBufferSource.BufferSource) : PictureInPictureRenderer<NVGPIPRenderer.NVGRenderState>(vertexConsumers) {
    ...
    override fun renderToTexture(state: NVGRenderState, poseStack: PoseStack) {
        ...
        val bufferManager = getDirectStateAccess() ?: return
    ...
    fun getDirectStateAccess(): DirectStateAccess? {
        val device = RenderSystem.getDevice()
        val backendField = GpuDevice::class.java.getDeclaredField("backend")
        backendField.isAccessible = true
        val backend = backendField.get(device)

        val glDeviceClass = Class.forName("com.mojang.blaze3d.opengl.GlDevice")
        if (glDeviceClass.isInstance(backend)) {
            val method = glDeviceClass.getMethod("directStateAccess")
            return method.invoke(backend) as DirectStateAccess
        }
        return null
    }
