...
object HuntingHelper : Module("Hunting Helper", "Features for creature hunting.") {
    ...
    private val floordropEsp by BooleanSetting("Floordrop ESP", "Highlight floordrop in Safari.", false).depends { safari }
    private val duplicoEsp by BooleanSetting("Duplico ESP", "Highlight Duplico in safari.", true).depends { safari }
    ...
    private val huntingMobs = listOf(
        HuntingMob("Hideonsun", EntityTypes.SHULKER, Colors.MINECRAFT_GOLD, { hideonsunEsp },
            area = Island.TorrhusCanyon,
            extra = { val c = (it as Shulker).color; c == DyeColor.YELLOW || c == DyeColor.ORANGE || c == DyeColor.BROWN }),
        HuntingMob("Hideonwall", EntityTypes.SHULKER, Colors.MINECRAFT_DARK_PURPLE, { hideonwallEsp },
            area = Island.Safari,
            extra = { (it as Shulker).color == DyeColor.PURPLE }),
        HuntingMob("Hideonfloor", EntityTypes.SHULKER, Colors.MINECRAFT_DARK_GREEN, { hideonfloorEsp },
            area = Island.Safari,
            extra = { (it as Shulker).color == DyeColor.GREEN }),
        HuntingMob("Bloodbat", EntityTypes.BAT, Colors.MINECRAFT_RED, { bloodbatEsp },
            area = Island.Safari,
            extra = { SafariArea.getCurrentArea() == SafariArea.Haunted }),

        HuntingMob("Pangolin", EntityTypes.ARMADILLO, Color(0x9F5656), { pangolinEsp }),
        HuntingMob("BeeHeemoth", EntityTypes.BEE, Color(0xFECE3E), { beeHeemothEsp }, extra = { (it as Bee).scale == 9.0f })
    )
