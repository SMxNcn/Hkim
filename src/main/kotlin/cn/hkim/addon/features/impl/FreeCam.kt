...
@ModuleInfo("free_cam", category = Category.MISC)
object FreeCam : Module("Free Camera", "Detach your camera and fly around freely.") {
    ...
    private val toggleKey by KeybindSetting("Toggle FreeCam", "Key to toggle FreeCam.", InputConstants.KEY_F6)
    ...
    private fun onInput(event: InputEvent) {
        if (event.key.type != InputConstants.Type.KEYBOARD) return
        if (event.key.value == toggleKey) {
            toggle()
            event.cancel()
        } else if (event.key.value == InputConstants.KEY_F5 && isFreecamActive) {
            event.cancel()
        }
    }
    ...
    private fun startFreecam() {
        ...
        // Force terrain re-occlusion at the new camera position
        forceTerrainUpdate()
    ...
    private fun stopFreecam() {
        ...
        // Force terrain re-occlusion back at the player position
        forceTerrainUpdate()
