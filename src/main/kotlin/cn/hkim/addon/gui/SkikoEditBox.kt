...
class SkikoEditBox(
    ...
    var selectionAnchor: Int = -1
        private set
    private var focusedInternal = false
        set(value) {
            if (field == value) return
            field = value
            // 26.3: 文本输入焦点通过 GuiEventListener 实例告知（SDL 事件循环会据此路由 IME/剪贴板事件）
            mc.onTextInputFocusChange(this, value)
            if (!value) {
                dragging = false
                dragWordMode = false
                selectionAnchor = -1
    ...
    override fun isFocused(): Boolean = focusedInternal

    override fun setFocused(focused: Boolean) {
        focusedInternal = focused
    }
    ...
    fun handleKeyPressed(event: KeyEvent): Boolean {
        ...
        when {
            ...
            event.key() == InputConstants.KEY_HOME -> { moveCursorTo(0, shift); return true }
            event.key() == InputConstants.KEY_END -> { moveCursorTo(text.length, shift); return true }
            event.key() == InputConstants.KEY_BACKSPACE -> { if (ctrl) deleteWordBackward() else deleteBackward(); return true }
            event.key() == InputConstants.KEY_DELETE -> { if (ctrl) deleteWordForward() else deleteForward(); return true }
            else -> return false
        }
    }
    ...
    private fun copySelection() {
        ...
        SDLClipboard.SDL_SetClipboardText(text.substring(s, e))
    }
    ...
    private fun paste() {
        val clipboard = SDLClipboard.SDL_GetClipboardText() ?: return
