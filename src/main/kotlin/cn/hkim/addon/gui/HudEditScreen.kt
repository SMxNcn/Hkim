...
class HudEditScreen(private val parent: Screen?) : Screen(Component.literal("HUD Editor")) {
    ...
    override fun keyPressed(event: KeyEvent): Boolean {
        ...
        val step = if (event.modifiers() and InputConstants.MOD_SHIFT != 0) 10f else 1f
        when (event.key) {
            InputConstants.KEY_LEFT -> el.anchorX -= step
            InputConstants.KEY_RIGHT -> el.anchorX += step
            InputConstants.KEY_UP -> el.anchorY -= step
            InputConstants.KEY_DOWN -> el.anchorY += step
