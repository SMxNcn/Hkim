...
class ActionInputScreen(
    ...
    override fun mouseClicked(mouseButtonEvent: MouseButtonEvent, bl: Boolean): Boolean {
        ...
        if (mouseButtonEvent.button() == InputConstants.MOUSE_BUTTON_LEFT && inBox) {
            leftClick = !leftClick
            return true
        }
    ...
    override fun keyPressed(keyEvent: KeyEvent): Boolean {
        return when (keyEvent.key) {
            InputConstants.KEY_W -> { forward = !forward; true }
            InputConstants.KEY_A -> { left = !left; true }
            InputConstants.KEY_S -> { back = !back; true }
            InputConstants.KEY_D -> { right = !right; true }
            InputConstants.KEY_ESCAPE -> { onClose(); true }
